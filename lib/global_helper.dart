...
class GlobalHelper {
  ...
  Future<dynamic> updatePassword(
      String email, String current_password, String new_password) async {
    var response = await http.post(
  ...
  Future<Map<String, dynamic>> get_dashboard_data() async {
    final response = await http.get(Uri.parse('${api}/get_dashboard_data'));
  ...
  Future<Map<String, dynamic>> get_orders(outlet_id) async {
    var response = await http.get(
      Uri.parse('${api}/get_orders?outlet_id=${outlet_id}'),
    );
    if (response.statusCode == 200) {
      final jsonData = jsonDecode(response.body);
      return jsonData;
